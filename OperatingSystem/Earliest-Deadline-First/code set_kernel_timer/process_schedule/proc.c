 ...
 */
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	...
	if (q == 0) {
		...
		tmp = rp->p_nextready;
		while (tmp) {
			if (tmp->ddl_est.tmr_exp_time > 0 && (rp->ddl_est.tmr_exp_time == 0
					|| (rp->ddl_est.tmr_exp_time > 0
					&& tmp->ddl_est.tmr_exp_time < rp->ddl_est.tmr_exp_time)))
				rp = tmp;
			tmp = tmp->p_nextready;
		}
	}
