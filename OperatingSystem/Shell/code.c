...
enum functions_name {read_from_file, write_to_file, write_append_to_file, pipeline};
char *historys[MAXN];
int his_number=0;
...
int Change_IO(char **argv, int argc, int *func, int *pos, int func_num, int *index);
...
int main()
{
	signal(SIGCHLD, sig_child);		/* reap zombie */
...
void Evaluate(char *input)
{
	...
	if((pid = fork()) == 0){		/* child process */
		...
		/* four special functions, judging whether need to change stdin or stdout */
		if(func_num)
...
/* parse the cmd_line , fulfill arguments, check for background pattern and find the position of each unique functions */
int Parse(char *buf, char **argv, int *argc, int *func, int *pos, int *func_num)
/* buf: input string.
   argv: buf separate to each word.
   argc: the number of argv without &. 
   func: store the index of functions. if func[0]=1, then functions[func[0]]="<".
   pos: the position of func[i].
   func_num: the number of functions. */
{
    int i;
	int background = 0;
    char *delim = " ";
    ...
    while((argv[++*argc] = strtok(NULL, delim))){
        for(i=0; i<4; i++){
            if(!strcmp(argv[*argc], functions[i])){
            	argv[*argc] = NULL;					/* if it is four of the special functions, then change it into NUNLL */
                func[*func_num] = i;				/* record what the function is */
                pos[(*func_num)++] = *argc;			/* record where the functions is in argv */
                break;
            }
        }
    }
		...
		if( ((pos[i]+1) < argc) && (argv[pos[i]+1] != NULL) ){
			...
			switch(f){
				...
				case write_to_file:
					...
					IO(fd, 1);
					break;
				case write_append_to_file:
					fd = open(argv[pos[i]+1], O_WRONLY|O_CREAT|O_APPEND);
					IO(fd, 1);
					break;
				case pipeline:
					Pipe(argv, func, pos, i);
					*index = i;				/* if pipe is the last cmd, then need to tell where the cmd starts */
...
void Pipe(char **argv, int *func, int *pos, int index)
{
	...
	if((pid = fork()) == 0){	/* child pattern */
		...
		if(fore)					/* there is a pipe before this one, so we need to finish the last one */
